...
Rules: read `AGENTS.md` first · obey `DESIGN.md` (tokens, no raw hex/px/font in `app/**`,`components/**`) · implement, don't redesign · flag ambiguity · verify each ticket: `npm.cmd exec tsc -- --noEmit` + `npm.cmd run lint` + `npm.cmd run build` + affected Playwright routes. Tick the box when done.
...
- [ ] **T1 — Command language v1** `src/components/layout/CommandPalette.tsx`
...
- [ ] **T2 — Keyboard system + `?` overlay** `Nav.tsx`, `CommandPalette.tsx`, new shortcut host
...
- [ ] **T5 — Inline provenance dot on numeric cells** tool tables + `src/components/ui/index.tsx`
...
- [ ] **T6 — Analyst-arc ordering + entity-aware NextLinks** `Nav.tsx`, all tool pages
...
- [ ] **T7 — Cleanup** `Nav.tsx`, `CommandPalette.tsx`, `globals.css`
...
- [ ] **T9 — Clickable cross-linking / entity pivot** tool tables, `CalculationTrace.tsx`
...
- [ ] **T12 — Company switcher + "Coverage" set** `Nav.tsx` dataset badge, `global-data-store.ts`
...
- [ ] **T15 — Excel export with live formulas** `src/lib/memo-export.ts` (`xlsx` dep already present)
  Export the model to `.xlsx` with intact formulas (not just values).
  **Done:** exported sheet recomputes in Excel; values match app.

- [ ] **T16 — Command language v2** [architectural] `CommandPalette.tsx`, `Nav.tsx`
